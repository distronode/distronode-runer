ARG PYTHON_BASE_IMAGE=quay.io/distronode/python-base:latest
ARG PYTHON_BUILDER_IMAGE=quay.io/distronode/python-builder:latest
ARG DISTRONODE_BRANCH=""
ARG ZUUL_SIBLINGS=""
...
COPY . /tmp/src
RUN if [ "$DISTRONODE_BRANCH" != "" ] ; then \
      echo "Installing requirements.txt / upper-constraints.txt for Distronode $DISTRONODE_BRANCH" ; \
      cp /tmp/src/tools/requirements-$DISTRONODE_BRANCH.txt /tmp/src/requirements.txt ; \
      cp /tmp/src/tools/upper-constraints-$DISTRONODE_BRANCH.txt /tmp/src/upper-constraints.txt ; \
    else \
...
RUN for dir in \
    ...
    for file in \
      /home/runner/.distronode/galaxy_token \
      /etc/passwd \
      /etc/group ; \
...
CMD ["distronode-runner", "run", "/runner"]
