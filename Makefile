...
IMAGE_NAME ?= quay.io/distronode/distronode-runner
IMAGE_NAME_STRIPPED := $(word 1,$(subst :, ,$(IMAGE_NAME)))
GIT_BRANCH ?= $(shell git rev-parse --abbrev-ref HEAD)
DISTRONODE_BRANCH ?=
DISTRONODE_VERSIONS ?= stable-2.9 stable-2.10 stable-2.11
PIP_NAME = distronode_runner
...
clean:
	rm -rf dist
	rm -rf build
	rm -rf distronode-runner.egg-info
	rm -rf rpm-build
	rm -rf deb-build
...
image: sdist
	$(CONTAINER_ENGINE) build --rm=true \
		--build-arg DISTRONODE_BRANCH=$(DISTRONODE_BRANCH) \
		-t $(IMAGE_NAME) -f Dockerfile .
	$(CONTAINER_ENGINE) tag $(IMAGE_NAME) $(IMAGE_NAME_STRIPPED):$(GIT_BRANCH)

image_matrix: image
	for version in $(DISTRONODE_VERSIONS) ; do \
		DISTRONODE_BRANCH=$$version GIT_BRANCH=$$version-$(GIT_BRANCH) make image ; \
	done
	$(CONTAINER_ENGINE) tag $(IMAGE_NAME) $(IMAGE_NAME_STRIPPED):$(GIT_BRANCH)
