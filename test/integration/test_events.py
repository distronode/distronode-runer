...
def test_basic_events(containerized, runtime, tmp_path, container_image, is_run_async=False, g_facts=False):
    ...
    run_args = {'private_data_dir': str(tmp_path),
                'inventory': inventory,
                'envvars': {"DISTRONODE_DEPRECATION_WARNINGS": "False", 'DISTRONODE_PYTHON_INTERPRETER': 'auto_silent'},
                'playbook': playbook}
    if containerized:
...
def test_basic_serializeable(tmp_path):
    inv = 'localhost distronode_connection=local distronode_python_interpreter="{{ distronode_playbook_python }}"'
...
def test_include_role_events(project_fixtures):
    ...
    for event in role_events:
        ...
        if event['event'] == 'playbook_on_task_start':
            assert event_data['resolved_action'] == 'distronode.builtin.debug'
