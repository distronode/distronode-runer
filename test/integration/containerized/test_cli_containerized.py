...
def test_cli_kill_cleanup(cli, runtime, project_fixtures, container_image):
    ...
    def container_is_running():
        r = cli([runtime, 'ps', '-f', f'name=distronode_runner_{ident}', '--format={{.Names}}'], bare=True)
        return ident in r.stdout

    timeout = 10
    for _ in iterate_timeout(timeout, 'confirm distronode-runner started container', interval=1):
