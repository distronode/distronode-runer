...
from distronode_runner.cleanup import cleanup_images, prune_images
from distronode_runner.defaults import default_container_image
...
def test_cleanup_new_image(cli, runtime, tmp_path):
    ...
    # workaround for https://github.com/distronode/distronode-runner/issues/758
    tmp_path.joinpath('project').mkdir()

    # force no colors so that we can JSON load ad hoc output
    env_path = tmp_path.joinpath('env')
    env_path.mkdir()
    env_path.joinpath('envvars').write_text('{"DISTRONODE_NOCOLOR": "true"}')

    # assure that the image is usable in distronode-runner as an EE
    r = cli([
