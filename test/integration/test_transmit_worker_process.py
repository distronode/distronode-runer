...
import distronode_runner.interface  # AWX import pattern
...
class TestStreamingUsage:
    ...
    def check_artifacts(process_dir, job_type):
        ...
        if job_type == 'run':
            ...
            assert '"distronode_facts"' in stdout
    ...
    def test_keepalive_setting(self, tmp_path, project_fixtures, keepalive_setting):
        ...
        if keepalive_setting is None:
            ...
            os.environ['DISTRONODE_RUNNER_KEEPALIVE_SECONDS'] = '1'
        elif 'DISTRONODE_RUNNER_KEEPALIVE_SECONDS' in os.environ:
            # don't allow this envvar to affect the test behavior
            del os.environ['DISTRONODE_RUNNER_KEEPALIVE_SECONDS']
    ...
    def test_process_isolation_executable_not_exist(self, tmp_path, mocker):
        ...
        """
        mocker.patch.object(distronode_runner.interface, 'check_isolation_executable_installed', return_value=False)
        ...
        transmitter = distronode_runner.interface.run(
            streamer='transmit',
        ...
        with pytest.raises(SystemExit) as exc:
            distronode_runner.interface.run(
                streamer='worker',
