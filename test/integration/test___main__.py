...
import distronode_runner.__main__
...
def test_cmdline_role(options, expected_playbook, tmp_path, mocker):
    mocker.patch.object(distronode_runner.__main__, 'run', save_playbook)
    spy = mocker.spy(distronode_runner.__main__, 'run')

    command = ['run', str(tmp_path)]
    command.extend(options)

    rc = distronode_runner.__main__.main(command)
...
def test_cmdline_playbook(tmp_path):
    ...
    with open(inventory, 'w') as f:
        f.write('[all]\nlocalhost distronode_connection=local distronode_python_interpreter="{{ distronode_playbook_python }}"')
