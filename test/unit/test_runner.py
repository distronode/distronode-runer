...
from distronode_runner.exceptions import CallbackError, DistronodeRunnerException
...
def test_event_callback_data_check(rc, mocker):
    ...
    with pytest.raises(DistronodeRunnerException) as exc:
        runner.event_callback(dict(uuid="testuuid", counter=0))

    assert "Failed to open distronode stdout callback plugin partial data" in str(exc)
