...
def test_dump_artifacts_private_data_dir_create_tempfile(mocker):
    mocker.patch('distronode_runner.utils.os.path.exists', side_effect=AttributeError('Raised intentionally'))
    mocker.patch('distronode_runner.utils.tempfile.mkdtemp', return_value='/tmp/dir')
...
def test_dump_artifacts_playbook_object(mocker, playbook):
    mock_dump_artifact = mocker.patch('distronode_runner.utils.dump_artifact', side_effect=AttributeError('Raised intentionally'))
    mocker.patch('distronode_runner.utils.isplaybook', return_value=True)
...
def test_dump_artifacts_role(mocker):
    mock_dump_artifact = mocker.patch('distronode_runner.utils.dump_artifact')
    ...
    mock_dump_artifact.assert_called_with('{"DISTRONODE_ROLES_PATH": "/tmp/roles"}', '/tmp/env', 'envvars')
...
def test_dump_artifacts_roles_path(mocker):
    ...
    assert mock_dump_artifact.call_count == 2
    mock_dump_artifact.assert_called_with('{"DISTRONODE_ROLES_PATH": "/tmp/altrole:/tmp/roles"}', '/tmp/env', 'envvars')
