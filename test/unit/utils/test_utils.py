...
def test_args2cmdline():
    res = args2cmdline('distronode', '-m', 'setup', 'localhost')
...
def test_signal_handler(mocker):
    ...
    mocker.patch('distronode_runner.utils.threading.main_thread', return_value='thread0')
    mocker.patch('distronode_runner.utils.threading.current_thread', return_value='thread0')
    mocker.patch('distronode_runner.utils.threading.Event', MockEvent)
    mock_signal = mocker.patch('distronode_runner.utils.signal.signal')
...
def test_signal_handler_outside_main_thread(mocker):
    ...
    mocker.patch('distronode_runner.utils.threading.main_thread', return_value='thread0')
    mocker.patch('distronode_runner.utils.threading.current_thread', return_value='thread1')
