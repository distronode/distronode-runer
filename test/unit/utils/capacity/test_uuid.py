...
def mock_uuid(mocker):
    ...
    mocker.patch('distronode_runner.utils.capacity.uuid.uuid4', return_value=uuid)
...
def mock_home_path(mocker, tmp_path):
    mocker.patch('distronode_runner.utils.capacity.Path.home', return_value=tmp_path)


def test_set_uuid(mock_uuid, mock_home_path, tmp_path):
    uuid = _set_uuid()

    expected_path = tmp_path / '.distronode_runner_uuid'
...
def test_ensure_uuid(mocker, mock_uuid, mock_home_path, tmp_path):
    mock_set_uuid = mocker.patch('distronode_runner.utils.capacity._set_uuid', return_value=mock_uuid)

    uuid = ensure_uuid()

    assert uuid == mock_uuid
    mock_set_uuid.assert_called_with(tmp_path / '.distronode_runner_uuid', 0o600)
