...
def test_distronode_doc_defaults(tmp_path, patch_private_data_dir):
    rc = DocConfig()
...
def test_prepare_plugin_docs_command():
    ...
    expected_command = [get_executable_path('distronode-doc'), '-s', '-t', 'module', '--playbook-dir', '/tmp/test', 'copy', 'file']
...
def test_prepare_plugin_list_command():
    ...
    expected_command = [get_executable_path('distronode-doc'), '-F', '-t', 'module', '--playbook-dir', '/tmp/test', '-M', '/test/module']
    assert rc.command == expected_command
    assert rc.runner_mode == 'subprocess'
    assert rc.execution_mode == BaseExecutionMode.DISTRONODE_COMMANDS
