...
def test_prepare_env_distronode_vars(mocker, tmp_path):
...
def test_wrap_args_with_ssh_agent_defaults(tmp_path):
    ...
    res = rc.wrap_args_with_ssh_agent(['distronode-playbook', 'main.yaml'], f'{tmp_path}/sshkey')
    assert res == [
        'ssh-agent',
        'sh', '-c',
        f"trap 'rm -f {tmp_path}/sshkey' EXIT && ssh-add {tmp_path}/sshkey && rm -f {tmp_path}/sshkey && distronode-playbook main.yaml"
    ]


def test_wrap_args_with_ssh_agent_with_auth(tmp_path):
    rc = BaseConfig(private_data_dir=str(tmp_path))
    res = rc.wrap_args_with_ssh_agent(['distronode-playbook', 'main.yaml'], f'{tmp_path}/sshkey', f'{tmp_path}/sshauth')
    assert res == [
...
def test_wrap_args_with_ssh_agent_silent(tmp_path):
    rc = BaseConfig(private_data_dir=str(tmp_path))
    res = rc.wrap_args_with_ssh_agent(['distronode-playbook', 'main.yaml'], f'{tmp_path}/sshkey', silence_ssh_add=True)
    assert res == [
        'ssh-agent',
        'sh', '-c',
        f"trap 'rm -f {tmp_path}/sshkey' EXIT && ssh-add {tmp_path}/sshkey 2>/dev/null && rm -f {tmp_path}/sshkey && distronode-playbook main.yaml"
    ]
...
def test_container_volume_mounting_with_Z(tmp_path, mocker):
    ...
    rc.env = {}
    rc.execution_mode = BaseExecutionMode.DISTRONODE_COMMANDS
    rc.command = ['distronode-playbook', 'foo.yml']
...
def test_containerization_settings(tmp_path, runtime, mocker):
    ...
    mock_containerized = mocker.patch('distronode_runner.config._base.BaseConfig.containerized', new_callable=mocker.PropertyMock)
    mock_containerized.return_value = True

    rc = BaseConfig(private_data_dir=tmp_path)
    rc.ident = 'foo'
    rc.cmdline_args = ['main.yaml', '-i', '/tmp/inventory']
    rc.command = ['distronode-playbook'] + rc.cmdline_args
    ...
    expected_command_start.extend([
        ...
        'my_container', 'distronode-playbook', 'main.yaml', '-i', '/tmp/inventory',
    ])
...
def test_containerization_unsafe_write_setting(tmp_path, runtime, mocker):
    ...
    assert rc.env.get('DISTRONODE_UNSAFE_WRITES') == expected[runtime]
