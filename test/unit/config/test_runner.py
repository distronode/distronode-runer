...
def test_prepare_env_sshkey(mocker):
    mocker.patch('distronode_runner.config._base.open_fifo_write')
    mocker.patch('os.makedirs', return_value=True)
    rc = RunnerConfig('/')
...
def test_prepare_env_directory_isolation_from_settings(mocker, project_fixtures):
    ...
    mocker.patch('distronode_runner.config.runner.RunnerConfig.build_process_isolation_temp_dir')
...
@pytest.mark.parametrize(
    'extra_vars, expected',
    (
        ({'test': 'key'}, ['distronode-playbook', '-i', '/inventory', '-e', '@/env/extravars', '-e', '{"test":"key"}', 'main.yaml']),
        ('/tmp/extravars.yml', ['distronode-playbook', '-i', '/inventory', '-e', '@/env/extravars', '-e', '@/tmp/extravars.yml', 'main.yaml']),
        (None, ['distronode-playbook', '-i', '/inventory', '-e', '@/env/extravars', 'main.yaml']),
    )
)
def test_generate_distronode_command_extra_vars(mocker, extra_vars, expected):
    mocker.patch('os.makedirs', return_value=True)
...
def test_generate_distronode_command(mocker):
    mocker.patch('os.makedirs', return_value=True)
    mocker.patch('os.path.exists', return_value=True)
    ...
    cmd = rc.generate_distronode_command()
    assert cmd == ['distronode-playbook', '-i', 'localhost,', 'main.yaml']

    rc.inventory = ['thing1', 'thing2']
    cmd = rc.generate_distronode_command()
    assert cmd == ['distronode-playbook', '-i', 'thing1', '-i', 'thing2', 'main.yaml']

    rc.inventory = []
    cmd = rc.generate_distronode_command()
    assert cmd == ['distronode-playbook', 'main.yaml']
    ...
    rc.prepare_inventory()
    cmd = rc.generate_distronode_command()
    assert cmd == ['distronode-playbook', '-i', '/inventory', '-vvv', 'main.yaml']
    rc.verbosity = None

    rc.limit = 'hosts'
    cmd = rc.generate_distronode_command()
    assert cmd == ['distronode-playbook', '-i', '/inventory', '--limit', 'hosts', 'main.yaml']
    rc.limit = None

    rc.module = 'setup'
    cmd = rc.generate_distronode_command()
    assert cmd == ['distronode', '-i', '/inventory', '-m', 'setup']
    rc.module = None

    rc.module = 'setup'
    rc.module_args = 'test=string'
    cmd = rc.generate_distronode_command()
    assert cmd == ['distronode', '-i', '/inventory', '-m', 'setup', '-a', 'test=string']
    rc.module_args = None
    rc.module = None

    rc.forks = 5
    cmd = rc.generate_distronode_command()
    assert cmd == ['distronode-playbook', '-i', '/inventory', '--forks', '5', 'main.yaml']


def test_generate_distronode_command_with_api_extravars(mocker):
    mocker.patch('os.makedirs', return_value=True)
    ...
    cmd = rc.generate_distronode_command()
    assert cmd == ['distronode-playbook', '-i', '/inventory', '-e', '{"foo":"bar"}', 'main.yaml']


def test_generate_distronode_command_with_dict_extravars(mocker):
    mocker.patch('os.makedirs', return_value=True)
    ...
    cmd = rc.generate_distronode_command()
    assert cmd == ['distronode-playbook', '-i', '/inventory', '-e', '{"foo":"test \\n hello"}', 'main.yaml']
...
def test_generate_distronode_command_with_cmdline_args(cmdline, tokens, mocker):
    ...
    cmd = rc.generate_distronode_command()
    assert cmd == ['distronode-playbook'] + tokens + ['-i', '/inventory', 'main.yaml']
...
def test_prepare(mocker):
    ...
    rc.artifact_dir = '/'
    rc.env = {}
    rc.execution_mode = ExecutionMode.DISTRONODE_PLAYBOOK
    ...
    assert rc.env['DISTRONODE_HOST_KEY_CHECKING'] == 'False'
    assert rc.env['AWX_ISOLATED_DATA_DIR'] == '/'


def test_prepare_with_ssh_key(mocker):
    mocker.patch('os.makedirs', return_value=True)
    open_fifo_write_mock = mocker.patch('distronode_runner.config._base.open_fifo_write')
    rc = RunnerConfig('/')
...
def test_wrap_args_with_ssh_agent_defaults(mocker):
    mocker.patch('os.makedirs', return_value=True)
    rc = RunnerConfig('/')
    res = rc.wrap_args_with_ssh_agent(['distronode-playbook', 'main.yaml'], '/tmp/sshkey')
    assert res == [
        'ssh-agent',
        'sh', '-c',
        "trap 'rm -f /tmp/sshkey' EXIT && ssh-add /tmp/sshkey && rm -f /tmp/sshkey && distronode-playbook main.yaml"
    ]
...
def test_wrap_args_with_ssh_agent_with_auth(mocker):
    mocker.patch('os.makedirs', return_value=True)
    rc = RunnerConfig('/')
    res = rc.wrap_args_with_ssh_agent(['distronode-playbook', 'main.yaml'], '/tmp/sshkey', '/tmp/sshauth')
    assert res == [
...
def test_wrap_args_with_ssh_agent_silent(mocker):
    mocker.patch('os.makedirs', return_value=True)
    rc = RunnerConfig('/')
    res = rc.wrap_args_with_ssh_agent(['distronode-playbook', 'main.yaml'], '/tmp/sshkey', silence_ssh_add=True)
    assert res == [
        'ssh-agent',
        'sh', '-c',
        "trap 'rm -f /tmp/sshkey' EXIT && ssh-add /tmp/sshkey 2>/dev/null && rm -f /tmp/sshkey && distronode-playbook main.yaml"
    ]
...
def test_bwrap_process_isolation_defaults(mocker):
    ...
    assert rc.command == [
        ...
        '--bind', '/', '/',
        '--chdir', '/project',
        'distronode-playbook', '-i', '/inventory', 'main.yaml',
    ]
...
def test_bwrap_process_isolation_and_directory_isolation(mocker, patch_private_data_dir, tmp_path):
    ...
    mocker.patch('distronode_runner.config.runner.os.chmod', return_value=True)
    mocker.patch('distronode_runner.config.runner.os.path.exists', mock_exists)
    mocker.patch('distronode_runner.config._base.ArtifactLoader', new=MockArtifactLoader)
...
def test_process_isolation_settings(mocker, tmp_path):
    ...
    assert 'distronode_runner_pi' in rc.command[index + 1]
    assert rc.command[index + 2] == os.path.realpath('/home')  # needed for Mac

    # hide /var
    assert rc.command[index + 3] == '--bind'
    assert 'distronode_runner_pi' in rc.command[index + 4]
    ...
    # chdir and distronode-playbook command
    assert rc.command[index + 15:] == ['--chdir', '/project', 'distronode-playbook', '-i', '/inventory', 'main.yaml']
...
def test_containerization_settings(tmp_path, runtime, mocker):
    mocker.patch('os.path.isdir', return_value=True)
    mocker.patch('os.path.exists', return_value=True)
    mock_containerized = mocker.patch('distronode_runner.runner_config.RunnerConfig.containerized', new_callable=mocker.PropertyMock)
    ...
    # validate DISTRONODE_CALLBACK_PLUGINS env var is set
    assert rc.env.get('DISTRONODE_CALLBACK_PLUGINS', None) is not None
