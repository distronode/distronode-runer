...
def test_default_callback_set(mocker):
    mocker.patch('distronode_runner.interface.signal_handler', side_effect=AttributeError('Raised intentionally'))
...
def test_set_cancel_callback(mocker):
    mock_runner = mocker.patch('distronode_runner.interface.Runner', side_effect=AttributeError('Raised intentionally'))
    mock_runner_config = mocker.patch('distronode_runner.interface.RunnerConfig')
