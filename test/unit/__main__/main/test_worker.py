import distronode_runner.__main__ as distronode_runner__main__
import pytest


def test_worker_delete(mocker):
    mock_output = mocker.patch.object(distronode_runner__main__, 'output')
    mock_output.configure.side_effect = AttributeError('Raised intentionally')

    mock_register_for_cleanup = mocker.patch.object(distronode_runner__main__, 'register_for_cleanup')
    mock_rmtree = mocker.patch.object(distronode_runner__main__.shutil, 'rmtree')
    mock_mkdtemp = mocker.patch.object(distronode_runner__main__.tempfile, 'mkdtemp', return_value='some_tmp_dir')
    ...
    with pytest.raises(AttributeError, match='Raised intentionally'):
        distronode_runner__main__.main(sys_args)
