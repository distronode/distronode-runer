...
class Worker:
    def __init__(self, _input=None, _output=None, keepalive_seconds: float | None = None, **kwargs):
        ...
        if keepalive_seconds is None:  # if we didn't get an explicit int value, fall back to envvar
            ...
            keepalive_seconds = float(os.environ.get('DISTRONODE_RUNNER_KEEPALIVE_SECONDS', 0))
    ...
    def update_paths(self, kwargs):
        if kwargs.get('envvars'):
            if 'DISTRONODE_ROLES_PATH' in kwargs['envvars']:
                roles_path = kwargs['envvars']['DISTRONODE_ROLES_PATH']
                roles_dir = os.path.join(self.private_data_dir, 'roles')
                kwargs['envvars']['DISTRONODE_ROLES_PATH'] = os.path.join(roles_dir, roles_path)
    ...
    def run(self):
        ...
        try:
            ...
            self.kwargs['suppress_distronode_output'] = True
            ...
            r = distronode_runner.interface.run(**self.kwargs)
