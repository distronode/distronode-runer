...
import distronode_runner.plugins
...
from .exceptions import CallbackError, DistronodeRunnerException
...
class Runner:
    ...
    def event_callback(self, event_data):
        '''
        Invoked for every Distronode event to collect stdout with the event data and store it for
        later use
        '''
        ...
        if 'uuid' in event_data:
            ...
            try:
                ...
                try:
                    ...
                    msg = "Failed to open distronode stdout callback plugin partial data" \
                          f" file {partial_filename} with error {str(e)}"
                    debug(msg)
                    if self.config.check_job_event_data:
                        raise DistronodeRunnerException(msg) from e
                ...
                if self.event_handler is not None:
                    ...
                    should_write = True
                for plugin in distronode_runner.plugins:
                    distronode_runner.plugins[plugin].event_handler(self.config, event_data)
                if should_write:
    ...
    def status_callback(self, status):
        ...
        for plugin in distronode_runner.plugins:
            distronode_runner.plugins[plugin].status_handler(self.config, status_data)
        if self.status_handler is not None:
            self.status_handler(status_data, runner_config=self.config)

    def run(self):
        '''
        Launch the Distronode task configured in self.config (A RunnerConfig object), returns once the
        ...
        if hasattr(self.config, 'suppress_distronode_output'):
            suppress_distronode_output = self.config.suppress_distronode_output
        else:
            suppress_distronode_output = False
        ...
        stdout_handle = OutputEventFilter(stdout_handle, self.event_callback, suppress_distronode_output, output_json=self.config.json_mode)
        stderr_handle = OutputEventFilter(stderr_handle, self.event_callback, suppress_distronode_output, output_json=self.config.json_mode)
        ...
        if self.config.containerized:
            ...
            # option expecting should have already been written in distronode_runner.config.runner
    ...
    def stdout(self):
        '''
        Returns an open file handle to the stdout representing the Distronode run
        '''
        stdout_path = os.path.join(self.config.artifact_dir, 'stdout')
        if not os.path.exists(stdout_path):
            raise DistronodeRunnerException("stdout missing")
        return open(os.path.join(self.config.artifact_dir, 'stdout'), 'r')

    @property
    def stderr(self):
        '''
        Returns an open file handle to the stderr representing the Distronode run
        '''
        stderr_path = os.path.join(self.config.artifact_dir, 'stderr')
        if not os.path.exists(stderr_path):
            raise DistronodeRunnerException("stderr missing")
        return open(os.path.join(self.config.artifact_dir, 'stderr'), 'r')

    @property
    def events(self):
        '''
        A generator that will return all distronode job events in the order that they were emitted from Distronode
...

            {
               ...
               "event_data": {
                  ...
                  "res": {
                     "msg": "Test!",
                     "changed": false,
                     "_distronode_verbose_always": true,
                     "_distronode_no_log": false
                  },
                  "pid": 740,
        ...
        while not os.path.exists(event_path):
            ...
            if wait_time.total_seconds() > 60:
                raise DistronodeRunnerException(f"events directory is missing: {event_path}")
    ...
    def stats(self):
        '''
        Returns the final high level stats from the Distronode run
    ...
    def get_fact_cache(self, host):
        ...
        if self.config.fact_cache_type != 'jsonfile':
            raise Exception('Unsupported fact cache type.  Only "jsonfile" is supported for reading and writing facts from distronode-runner')
