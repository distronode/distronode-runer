...
from distronode_runner.defaults import registry_auth_prefix
...
class BaseExecutionMode(Enum):
    NONE = 0
    # run distronode commands either locally or within EE
    DISTRONODE_COMMANDS = 1
...
class BaseConfig:
    ...
    def prepare_env(self, runner_mode: str = 'pexpect') -> None:
        ...
        if self.containerized:
            ...
            self.container_name = f"distronode_runner_{sanitize_container_name(self.ident)}"
            ...
            if self.process_isolation_executable == 'podman':
                ...
                #    https://github.com/distronode/distronode/pull/73282
                #    https://github.com/distronode/distronode/issues/73310
                #    https://issues.redhat.com/browse/AAP-476
                self.env['DISTRONODE_UNSAFE_WRITES'] = '1'
            ...
            if self.fact_cache_type == 'jsonfile':
                self.env['DISTRONODE_CACHE_PLUGIN_CONNECTION'] = os.path.join(artifact_dir, 'fact_cache')
        ...
        self.suppress_distronode_output = self.settings.get('suppress_distronode_output', self.quiet)
        ...
        if self.containerized:
            ...
            # then append to env['DISTRONODE_CALLBACK_PLUGINS'] with the copied location.
            ...
            self.env['DISTRONODE_CALLBACK_PLUGINS'] = ':'.join(filter(None, (self.env.get('DISTRONODE_CALLBACK_PLUGINS'), container_callback_dir)))
        else:
            callback_dir = self.env.get('AWX_LIB_DIRECTORY', os.getenv('AWX_LIB_DIRECTORY', ''))
            if not callback_dir:
                callback_dir = get_callback_dir()
            self.env['DISTRONODE_CALLBACK_PLUGINS'] = ':'.join(filter(None, (self.env.get('DISTRONODE_CALLBACK_PLUGINS'), callback_dir)))
        ...
        if self.env.get('DISTRONODE_STDOUT_CALLBACK'):
            self.env['ORIGINAL_STDOUT_CALLBACK'] = self.env.get('DISTRONODE_STDOUT_CALLBACK')

        if is_adhoc:
            # force loading awx_display stdout callback for adhoc commands
            self.env["DISTRONODE_LOAD_CALLBACK_PLUGINS"] = '1'
            if 'AD_HOC_COMMAND_ID' not in self.env:
                self.env['AD_HOC_COMMAND_ID'] = '1'

        self.env['DISTRONODE_STDOUT_CALLBACK'] = 'awx_display'

        self.env['DISTRONODE_RETRY_FILES_ENABLED'] = 'False'
        if 'DISTRONODE_HOST_KEY_CHECKING' not in self.env:
            self.env['DISTRONODE_HOST_KEY_CHECKING'] = 'False'
        if not self.containerized:
            self.env['AWX_ISOLATED_DATA_DIR'] = self.artifact_dir

        if self.fact_cache_type == 'jsonfile':
            self.env['DISTRONODE_CACHE_PLUGIN'] = 'jsonfile'
            if not self.containerized:
                self.env['DISTRONODE_CACHE_PLUGIN_CONNECTION'] = self.fact_cache
        ...
        for arg in cmdline_args:
            if arg in ['-i', '--inventory', '--inventory-file']:
                ...
                try:
                    ...
                    # to return correct error from distronode-core
                    return None
    ...
    def _handle_distronode_cmd_options_bind_mounts(self, args_list: list[str], cmdline_args: list[str]) -> None:
        ...
        for value in self.command:
            if 'distronode-playbook' in value:
        ...
        for arg in cmdline_args:
            ...
            try:
                ...
                # invalid command, pass through for execution
                # to return valid error from distronode-core
                return
    ...
    def wrap_args_for_containerization(self,
        ...
        if execution_mode != BaseExecutionMode.NONE:
            if execution_mode == BaseExecutionMode.DISTRONODE_COMMANDS:
                self._handle_distronode_cmd_options_bind_mounts(new_args, cmdline_args)
        ...
        # this file will be written in distronode_runner.runner
