...
class CommandConfig(BaseConfig):
    ...
    :py:mod:`distronode_runner.runner.CommandConfig` object to launch and manage the invocation of
    command execution.

    Typically this object is initialized for you when using the standard ``run`_command` interfaces in :py:mod:`distronode_runner.interface`
    ...
    _DISTRONODE_NON_INERACTIVE_CMDS = (
        'distronode-config',
        'distronode-doc',
        'distronode-galaxy',
    )
    ...
    def _set_runner_mode(self):
        if self.input_fd is not None or self.executable_cmd.split(os.pathsep)[-1] in CommandConfig._DISTRONODE_NON_INERACTIVE_CMDS:
    ...
    def _prepare_command(self):
        """
        Determines if it is ``distronode`` command or ``generic`` command and generate the command line
        """
        if not self.executable_cmd:
            raise ConfigurationError("For CommandRunner 'executable_cmd' value is required")

        if self.executable_cmd.split(os.pathsep)[-1].startswith('distronode'):
