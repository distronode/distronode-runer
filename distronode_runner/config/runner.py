...
from distronode_runner.output import debug
from distronode_runner.utils import register_for_cleanup
...
class RunnerConfig(BaseConfig):
    ...
    :py:mod:`distronode_runner.runner.Runner` object to launch and manage the invocation of ``distronode``
    and ``distronode-playbook``

    Typically this object is initialized for you when using the standard ``run`` interfaces in :py:mod:`distronode_runner.interface`
    ...
    def __init__(self,
                 ...
                 extravars=None, suppress_output_file=False, suppress_distronode_output=False, process_isolation_path=None,
        ...
        self.suppress_distronode_output = suppress_distronode_output
        self.tags = tags
    ...
    def prepare(self):
        ...
        and setting early DISTRONODE_ environment variables.
        """
        # distronode_path = find_executable('distronode')
        # if distronode_path is None or not os.access(distronode_path, os.X_OK):
        #     raise ConfigurationError("Distronode not found. Make sure that it is installed.")
        ...
        if self.execution_mode == ExecutionMode.DISTRONODE_PLAYBOOK and self.playbook is None:
            raise ConfigurationError("Runner playbook required when running distronode-playbook")
        elif self.execution_mode == ExecutionMode.DISTRONODE and self.module is None:
            raise ConfigurationError("Runner module required when running distronode")
    ...
    def prepare_env(self):
        ...
        with existing values so the :py:class:`distronode_runner.runner.Runner` object can read and use them easily
        """
        ...
        if self.roles_path:
            if isinstance(self.roles_path, list):
                self.env['DISTRONODE_ROLES_PATH'] = ':'.join(self.roles_path)
            else:
                self.env['DISTRONODE_ROLES_PATH'] = self.roles_path
    ...
    def prepare_command(self):
        try:
            ...
            self.command = self.generate_distronode_command()

    def generate_distronode_command(self):
        """
        Given that the ``RunnerConfig`` preparation methods have been run to gather the inputs this method
        will generate the ``distronode`` or ``distronode-playbook`` command that will be used by the
        :py:class:`distronode_runner.runner.Runner` object to start the process
        """
        if self.binary is not None:
            base_command = self.binary
            self.execution_mode = ExecutionMode.RAW
        elif self.module is not None:
            base_command = 'distronode'
            self.execution_mode = ExecutionMode.DISTRONODE
        else:
            base_command = 'distronode-playbook'
            self.execution_mode = ExecutionMode.DISTRONODE_PLAYBOOK
        ...
        # Other parameters
        if self.execution_mode == ExecutionMode.DISTRONODE_PLAYBOOK:
            exec_list.append(self.playbook)
        elif self.execution_mode == ExecutionMode.DISTRONODE:
            exec_list.append("-m")
    ...
    def build_process_isolation_temp_dir(self):
        ...
        '''
        path = tempfile.mkdtemp(prefix='distronode_runner_pi_', dir=self.process_isolation_path)
