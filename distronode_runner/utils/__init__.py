...
def dump_artifacts(kwargs):
    ...
    if 'role' in kwargs:
        ...
        kwargs['envvars']['DISTRONODE_ROLES_PATH'] = roles_path
...
class OutputEventFilter(object):
    ...
    def __init__(self, handle, event_callback,
                 suppress_distronode_output=False, output_json=False):
    ...
    def write(self, data):
        ...
        while should_search:
            ...
            if stdout_actual and stdout_actual != "{}":
                if not self.suppress_distronode_output:
                    sys.stdout.write(
...
def sanitize_json_response(data):
    '''
    Removes warning message from response message emitted by Distronode
