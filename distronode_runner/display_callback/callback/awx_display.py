...
#
# This file is part of Distronode Tower, but depends on code imported from Distronode.
#
...
DOCUMENTATION = '''
    ...
    short_description: Playbook event dispatcher for distronode-runner
    version_added: "2.0"
    description:
        - This callback is necessary for distronode-runner to work
    type: stdout
...
from distronode.plugins.loader import callback_loader
...
class DistronodeJSONEncoderLocal(json.JSONEncoder):
    '''
    The class DistronodeJSONEncoder exists in Distronode core for this function
    this performs a mostly identical function via duck typing
    '''

    def default(self, o):
        '''
        Returns JSON-valid representation for special Distronode python objects
        ...
        if getattr(o, 'yaml_tag', None) == '!vault':
            ...
            return {'__distronode_vault': encrypted_form}
        elif isinstance(o, (datetime.date, datetime.datetime)):
            return o.isoformat()
        return super(DistronodeJSONEncoderLocal, self).default(o)
...
class IsolatedFileWrite:
    ...
    def set(self, key, value):
        ...
        partial_data = json.dumps(value, cls=DistronodeJSONEncoderLocal)
...
class CallbackModule(DefaultCallbackModule):
    '''
    Callback module for logging distronode/distronode-playbook events.
    '''
    ...
    def capture_event_data(self, event, **event_data):
        ...
        if event_data.get('res'):
            if event_data['res'].get('_distronode_no_log', False):
            ...
            for i, item in enumerate(event_data['res'].get('results', [])):
                if isinstance(item, dict) and item.get('_distronode_no_log', False):
    ...
    def set_playbook(self, playbook):
        # NOTE: Distronode doesn't generate a UUID for playbook_on_start so do it for them.
    ...
    def v2_playbook_on_import_for_host(self, result, imported_file):
        # NOTE: Not used by Distronode 2.x.
    ...
    def v2_playbook_on_notify(self, handler, host):
        # NOTE: Not used by Distronode < 2.5.
        event_data = dict(
            host=host.get_name(),
    ...
    '''
    distronode_stats is, retroactively, added in 2.2
    '''
    ...
    def _get_event_loop(task):
        if hasattr(task, 'loop_with'):  # Distronode >=2.5
            return task.loop_with
        elif hasattr(task, 'loop'):  # Distronode <2.4
            return task.loop
        return None
    ...
    def v2_runner_on_ok(self, result):
        ...
        if result._task.action in ('setup', 'gather_facts'):
            result._result.get('distronode_facts', {}).pop('distronode_env', None)
