# Copyright (c) 2016 Distronode by Red Hat, Inc.
#
...
from distronode_runner.config.command import CommandConfig
...
from distronode_runner.config.doc import DocConfig
...
logging.getLogger('distronode-runner').addHandler(logging.NullHandler())
...
def init_runner(**kwargs):
    ...
    See parameters given to :py:func:`distronode_runner.interface.run`
    '''
    ...
    if kwargs.get('streamer'):
        ...
        roles_path = kwargs.get('envvars', {}).get('DISTRONODE_ROLES_PATH') or ''
        if os.path.isabs(roles_path) and roles_path.startswith(private_data_dir):
            kwargs['envvars']['DISTRONODE_ROLES_PATH'] = os.path.relpath(roles_path, private_data_dir)
...
def run(**kwargs):
    '''
    Run an Distronode Runner task in the foreground and return a Runner object when complete.
    ...
    :param str or list playbook: The playbook (either a list or dictionary of plays, or as a path relative to
                     ``private_data_dir/project``) that will be invoked by runner when executing Distronode.
    :param str module: The module that will be invoked in ad-hoc mode by runner when executing Distronode.
    ...
    :param dict or list roles_path: Directory or list of directories to assign to DISTRONODE_ROLES_PATH
    :param dict envvars: Environment variables to be used when running Distronode. Environment variables will also be
                    read from ``env/envvars`` in ``private_data_dir``
    :param dict extravars: Extra variables to be passed to Distronode at runtime using ``-e``. Extra vars will also be
    ...
    :param dict passwords: A dictionary containing password prompt patterns and response values used when processing output from
                      Distronode. Passwords will also be read from ``env/passwords`` in ``private_data_dir``.
    :param dict settings: A dictionary containing settings values for the ``distronode-runner`` runtime environment. These will also
                     be read from ``env/settings`` in ``private_data_dir``.
    :param str ssh_key: The ssh private key passed to ``ssh-agent`` as part of the distronode-playbook run.
    :param str cmdline: Command line options passed to Distronode read from ``env/cmdline`` in ``private_data_dir``
    :param bool suppress_env_files: Disable the writing of files into the ``env`` which may store sensitive information
    :param str limit: Matches distronode's ``--limit`` parameter to further constrain the inventory to be used
    :param int forks: Control Distronode parallel concurrency
    :param int verbosity: Control how verbose the output of distronode-playbook is
    ...
    :param int timeout: The timeout value in seconds that will be passed to either ``pexpect`` of ``subprocess`` invocation
                    ...
                    execution.
    :param str streamer: Optionally invoke distronode-runner as one of the steps in the streaming pipeline
    ...
    :param str container_image: Container image to use when running an distronode task (default: quay.io/distronode/distronode-runner:devel)
    ...
    :param bool omit_event_data: Omits extra distronode event data from event payload (stdout and event still included)
    :param bool only_failed_event_data: Omits extra distronode event data unless it's a failed event (stdout and event still included)
    :param bool check_job_event_data: Check if job events data is completely generated. If event data is not completely generated and if
                                 value is set to 'True' it will raise 'DistronodeRunnerException' exception,
                                 if set to 'False' it log a debug message and continue execution. Default value is 'False'

    :returns: A :py:class:`distronode_runner.runner.Runner` object, or a simple object containing ``rc`` if run remotely
    '''
...
def run_async(**kwargs):
    '''
    Runs an Distronode Runner task in the background which will start immediately. Returns the thread object and a Runner object.

    This uses the same parameters as :py:func:`distronode_runner.interface.run`

    :returns: A tuple containing a :py:class:`threading.Thread` object and a :py:class:`distronode_runner.runner.Runner` object
    '''
    r = init_runner(**kwargs)
...
def init_command_config(executable_cmd, cmdline_args=None, **kwargs):
    ...
    See parameters given to :py:func:`distronode_runner.interface.run_command`
    '''
    event_callback_handler = kwargs.pop('event_handler', None)
    status_callback_handler = kwargs.pop('status_handler', None)
    artifacts_handler = kwargs.pop('artifacts_handler', None)
...
def run_command(executable_cmd, cmdline_args=None, **kwargs):
    '''
    Run an (Distronode) commands in the foreground and return a Runner object when complete.
    ...
    :param str runner_mode: The applicable values are ``pexpect`` and ``subprocess``. If the value of ``input_fd`` parameter
                        is set or the executable command is one of ``distronode-config``, ``distronode-doc`` or ``distronode-galaxy``
...
def run_command_async(executable_cmd, cmdline_args=None, **kwargs):
    '''
    Run an (Distronode) commands in the background which will start immediately. Returns the thread object and a Runner object.

    This uses the same parameters as :py:func:`distronode_runner.interface.run_command`
...
def init_plugin_docs_config(plugin_names, plugin_type=None, response_format=None,
    ...
    See parameters given to :py:func:`distronode_runner.interface.get_plugin_docs`
    '''
...
def get_plugin_docs(plugin_names, plugin_type=None, response_format=None, snippet=False, playbook_dir=None, module_path=None, **kwargs):
    '''
    Run an distronode-doc command to get plugin docs  in the foreground and return a Runner object when complete.
    ...
    :param module_path: This parameter is prepend colon-separated path(s) to module library
                        (default=~/.distronode/plugins/modules:/usr/share/distronode/plugins/modules).
    ...
    :param host_cwd: The host current working directory to be mounted within the container (if enabled) and will be
                     the work directory within container.
    :param envvars: Environment variables to be used when running Distronode. Environment variables will also be
                    read from ``env/envvars`` in ``private_data_dir``
    :param passwords: A dictionary containing password prompt patterns and response values used when processing output from
                      Distronode. Passwords will also be read from ``env/passwords`` in ``private_data_dir``.
    :param settings: A dictionary containing settings values for the ``distronode-runner`` runtime environment. These will also
                     be read from ``env/settings`` in ``private_data_dir``.
    :param ssh_key: The ssh private key passed to ``ssh-agent`` as part of the distronode-playbook run.
    ...
    :param container_image: Container image to use when running an distronode task (default: quay.io/distronode/distronode-runner:devel)
...
def get_plugin_docs_async(plugin_names, plugin_type=None, response_format=None, snippet=False, playbook_dir=None, module_path=None, **kwargs):
    '''
    Run an distronode-doc command in the background which will start immediately. Returns the thread object and a Runner object.

    This uses the same parameters as :py:func:`distronode_runner.interface.get_plugin_docs`
...
def get_plugin_list(list_files=None, response_format=None, plugin_type=None, playbook_dir=None, module_path=None, **kwargs):
    '''
    Run an distronode-doc command to get list of installed Distronode plugins.
...
def get_inventory(action, inventories, response_format=None, host=None, playbook_dir=None,
    ...
    '''
    Run an distronode-inventory command to get inventory related details.
    ...
    :param export: The boolean value if set represent in a way that is optimized for export,not as an accurate
                   representation of how Distronode has processed it.
    :param runner_mode: The applicable values are ``pexpect`` and ``subprocess``. Default is set to ``subprocess``.
    :param host_cwd: The host current working directory to be mounted within the container (if enabled) and will be
                     the work directory within container.
    :param envvars: Environment variables to be used when running Distronode. Environment variables will also be
...
def get_distronode_config(action, config_file=None, only_changed=None, **kwargs):
    '''
    Run an distronode-config command to get distronode configuration releated details.
    ...
    :param envvars: Environment variables to be used when running Distronode. Environment variables will also be
                    read from ``env/envvars`` in ``private_data_dir``
    :param passwords: A dictionary containing password prompt patterns and response values used when processing output from Distronode.
    ...
    rd = DistronodeCfgConfig(**kwargs)
    rd.prepare_distronode_config_command(action=action, config_file=config_file, only_changed=only_changed)
    r = Runner(rd,
               event_handler=event_callback_handler,
               status_handler=status_callback_handler,
               artifacts_handler=artifacts_handler,
...
def get_role_list(collection=None, playbook_dir=None, **kwargs):
    '''
    Run an ``distronode-doc`` command to get list of installed collection roles.
    ...
    :param Callable artifacts_handler: An optional callback that will be invoked at the end of the run to deal with the artifacts from the run.
    :param bool check_job_event_data: Check if job events data is completely generated. If event data is not completely generated and if
        value is set to 'True' it will raise 'DistronodeRunnerException' exception. If set to 'False', log a debug message and continue execution.
        Default value is 'False'

    :returns: A tuple of response and error string. The response is a dictionary object
        (as returned by distronode-doc JSON output) containing each role found, or an empty dict
        if none are found.
    '''
    event_callback_handler = kwargs.pop('event_handler', None)
    status_callback_handler = kwargs.pop('status_handler', None)
...
def get_role_argspec(role, collection=None, playbook_dir=None, **kwargs):
    '''
    Run an ``distronode-doc`` command to get a role argument specification.
