...
# callback plugins under ~/.distronode, it must have prior knowledge of the user's homedir before the container is launched
# in order to know where to mount in the callback dir. In all cases, we must get a consistent answer from $HOME
# and anything that queries /etc/passwd for a homedir (eg, `~root`), or lots of things (including parts of Distronode
...
# primary GID 0), we need to correct that in order for distronode-runner's callbacks to function properly. Some things
...
if [[ -n "${LAUNCHED_BY_RUNNER}" ]]; then
    # Special actions to be compatible with old distronode-runner versions, 2.1.x specifically
    RUNNER_CALLBACKS=$(python3 -c "from distronode_runner.display_callback.callback import awx_display; print(awx_display.__file__)")
    export DISTRONODE_CALLBACK_PLUGINS="$(dirname $RUNNER_CALLBACKS)"

    # old versions split the callback name between awx_display and minimal, but new version just uses awx_display
    export DISTRONODE_STDOUT_CALLBACK=awx_display
fi

if [[ -d ${AWX_ISOLATED_DATA_DIR} ]]; then
    if output=$(distronode-galaxy collection list --format json 2> /dev/null); then
        echo $output > ${AWX_ISOLATED_DATA_DIR}/collections.json
    fi
    distronode --version 2> /dev/null | head -n 1 > ${AWX_ISOLATED_DATA_DIR}/distronode_version.txt
fi
